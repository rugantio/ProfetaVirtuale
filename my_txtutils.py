...
# »=9
# space=10
# all chars from 32 to 126 = c-22 --> [10, 104]
# LF mapped to 105
...
def print_learning_learned_comparison(X, Y, losses, bookranges, batch_loss, batch_accuracy, epoch_size, index, epoch):
    ...
    for k in range(batch_size):
        ...
        epoch_string = "{:3d}".format(index) + " (epoch {}) ".format(epoch)
        loss_string = "loss: {:.5f}".format(losses[k])
        print_string = epoch_string + formatted_bookname + " │ {:.55} │ {:.55} │ {}"
...
def read_data_files(directory, validation=True):
    ...
    codetext = []
    bookranges = []
    biblelist = sorted(glob.glob(directory, recursive=True))
    for biblefile in biblelist:
        bibletext = open(biblefile, "r")
        print("Loading file " + biblefile)
        start = len(codetext)
        codetext.extend(encode_text(bibletext.read()))
        end = len(codetext)
        bookranges.append({"start": start, "end": end, "name": biblefile.rsplit("/", 1)[-1]})
        bibletext.close()
...
def print_text_generation_header():
    print()
    print("┌{:─^158}┐".format('Generating random text from learned state'))


def print_text_generation_footer():
    print()
    print("└{:─^158}┘".format('End of generation'))
